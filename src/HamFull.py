...
class HamFull:
    ...
    def __init__( self, Ham, cluster_size, umat_bulk, umat_imp ):
        self.Ham  = Ham
        self.Tmat = self.ConstructFullTmat( cluster_size, umat_bulk, umat_imp )
        ...
        for count in range(0, numSteps):
            ...
            for count2 in range(0, numClusterOrbs):
                ...
                for count3 in range(0, numClusterOrbs):
                    ...
                    Tmat[ coordinate1, coordinate2 ] = self.Ham.getTmat( coordinate1, coordinate2 ) + umat_bulk[ count2, count3 ]
        
        # Overwrite the u-matrix in the impurity
        for count2 in range(0, numClusterOrbs):
            microshifts = np.zeros( [self.Ham.dim], dtype=int )
            copycount2 = count2
            for dim in range(0, self.Ham.dim):
                microshifts[dim] = copycount2 % cluster_size[dim]
                copycount2 = ( copycount2 - microshifts[dim] ) / cluster_size[dim]
            coordinate1 = self.Ham.getLinearCoordinate( microshifts )
            #print "Element",np.multiply(macroshifts, cluster_size)+microshifts
            for count3 in range(0, numClusterOrbs):
                microshifts2 = np.zeros( [self.Ham.dim], dtype=int )
                copycount3 = count3
                for dim in range(0, self.Ham.dim):
                    microshifts2[dim] = copycount3 % cluster_size[dim]
                    copycount3 = ( copycount3 - microshifts2[dim] ) / cluster_size[dim]
                coordinate2 = self.Ham.getLinearCoordinate( microshifts2 )
                Tmat[ coordinate1, coordinate2 ] = self.Ham.getTmat( coordinate1, coordinate2 ) + umat_imp[ count2, count3 ]
        return Tmat
